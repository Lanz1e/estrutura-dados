class Arco():
    ...
    def oposto(self,cidade):
        if cidade!=self.vertices[0]:
...
class Grafo():
    ...
    def adicionar_vertice(self, cidade):
        self.vertice=self.vertice+(cidade,)
    ...
    def caminho(self,cidade_1,cidade_2):
        cidades=[]
        if cidade_1==cidade_2:
            cidades.append(cidade_1)
            return cidades
        elif self.arcos(cidade_1)==tuple():
            return cidades
        else:
            atual=cidade_1
            while atual!=cidade_2:
                if self.arcos(atual)==tuple():
                    break
                for i in self.arcos(atual):
                    if i.oposto(atual) not in cidades:
